    ...
    <div class="board_wrap">
        ...
        <div class="container">
            <div class="dropdown">
                ...
                <ul class="dropdown-menu">
                    <li><a href="noticeboard.php">전체보기</a></li>
                    <li><a href="noticeboard_tagpage.php?bod_tag=자유">자유</a></li>
                    <li><a href="noticeboard_tagpage.php?bod_tag=선수">선수</a></li>
                    <li><a href="noticeboard_tagpage.php?bod_tag=인터뷰">인터뷰</a></li>
                    <li><a href="noticeboard_tagpage.php?bod_tag=일상">일상</a></li>
                    <li><a href="noticeboard_tagpage.php?bod_tag=후기">후기</a></li>
                    <li><a href="noticeboard_tagpage.php?bod_tag=질문">질문</a></li>
                </ul>
            </div>
        </div>
        <br>
        <div class="board_list_wrap">
            <div class="board_list">
                ...
                $sql = "SELECT * FROM board WHERE bod_tag = '$category' ORDER BY idx DESC ";
                ...
                $query = " SELECT * FROM board WHERE bod_tag = '$category' order by idx DESC LIMIT $page_start , $list ";
            ...
            <div id="page_num" style="text-align: center; margin-top:15px;">
                ...
                if ($page <= 1) {
                    ...
                    $pre = $page - 1; ?>
                    <a href="noticeboard_tagpage.php?bod_tag=<?= $category ?>&page=<?= $pre ?>" style="font-size:20px; text-decoration:none;"> ◀ 이전 &nbsp;&nbsp; </a>
                    <?php }
                for ($i = $block_start; $i <= $block_end; $i++) {
                    if ($page == $i) { ?>
                        <b style="font-size: 20px;">&nbsp; <?= $i ?> </b>
                    <?php } else { ?>
                        <a href="noticeboard_tagpage.php?bod_tag=<?= $category ?>&page=<?= $i ?>" style="font-size: 20px; text-decoration:none;">&nbsp; <?= $i ?> </a>
                    <?php }
                }
                ...
                if ($page >= $total_page) {
                    ...
                    $next = $page + 1; ?>
                    <a href="noticeboard_tagpage.php?bod_tag=<?= $category ?>&page=<?= $next ?>" style="font-size: 20px; text-decoration:none;"> &nbsp;&nbsp; 다음 ▶</a>

                <?php }
                if ($page >= $total_page) {
                    //빈 값 
                } else { ?>
                    <a href="noticeboard_tagpage.php?bod_tag=<?= $category ?>&page=<?= $total_page ?>" style="font-size: 20px; text-decoration:none;">&nbsp;&nbsp; 마지막</a>
                <?php } ?>
            ...
            <div class="board_searchpage" style="font-size: 14px;">
                <form method="GET" action="noticeboard_tag_searchpage.php">
                    <select name="category">
                        <option value="bod_nick">작성자</option>
                        <option value="content">내용</option>
                        <option value="title">제목</option>
                    </select>
                    <input type="hidden" name="bod_tag" value="<?= $category ?>">
